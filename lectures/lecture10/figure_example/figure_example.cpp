...
Не меняя функцию main, реализуйте недостающие функции и классы:
	...
	1) базовый класс IFigure с чисто виртуальными методами Name, Perimeter и Area;
	2) классы Triangle, Rect и Circle, которые являются наследниками класса IFigure и переопределяют его виртуальные методы;
...
std::shared_ptr<IFigure> CreateFigure(std::istringstream& is)
{
	std::string figure;
	is >> figure;
	if (figure == "RECT")
